...
const AddShows = () => {
  ...
  const { user, getToken, axios, imageBaseUrl } = useAppContext();

  const [nowPlayingMovies, setNowPlayingMovies] = useState([]);
  const [selectedMovie, setSelectedMovie] = useState(null);
  const [dateTimeSeletion, setDateTimeSelection] = useState({});
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (!selectedMovie || Object.keys(dateTimeSeletion).length === 0 || !showPrice) {
        return toast('Missing required fields')
      }

      const showsInput = Object.entries(dateTimeSeletion).map(([date, time]) => ({ date, time }));
  ...
  return nowPlayingMovies.length > 0 ? (
    <>
      ...
      {Object.keys(dateTimeSeletion).length > 0 && (
        <div className="mt-6">
          <h1 className="mb-2">Selected Date - Time</h1>
          <ul className="space-y-3">
            {Object.entries(dateTimeSeletion).map(([date, times]) => (
              <li key={date}>
